...
from modeling_dpt import GPTNeoXForCausalLM
...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    print(get_key(loaded_config, "mlp_type"))

    hf_model = GPTNeoXForCausalLM(hf_config)
    print(f"Model layers: {hf_model.gpt_neox.layers}")
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        print(f"Converting layer {layer_i+2}...")
        

        # get layer from hf model
        hf_layer = hf_model.gpt_neox.layers[layer_i]
        print(f"required state dict: {hf_layer.state_dict().keys()}")
    ...
    hf_model.gpt_neox.final_layer_norm.load_state_dict(
        {
            "scale": (sum([t["norm.scale"] for t in loaded_tp_ranks]))
            / len(loaded_tp_ranks),
        }
    )
    del loaded_tp_ranks
